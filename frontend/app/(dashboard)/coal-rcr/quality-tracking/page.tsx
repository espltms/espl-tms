...
export default function QualityTrackingPage() {
  ...
  const handleOpenAdd = () => {
    if (doRecords.length === 0) {
      setToast({ message: "Please configure a DO in the DO Master first!", type: 'error' });
      return;
    }
    if (rrRecords.length === 0) {
      setToast({ message: "Please configure RRs in the RR Entry first!", type: 'error' });
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        const errData = await response.json();
        setToast({ message: errData.error || "Failed to save Quality record.", type: 'error' });
        return;
      }
      ...
      setToast({ message: "An error occurred while saving the Quality record.", type: 'error' });
    }
  };
  ...
  const executeSingleDelete = async (id: string) => {
    try {
      ...
      if (response.ok) {
        setToast({ message: "Quality record successfully deleted.", type: 'success' });
        fetchData();
      } else {
        const errData = await response.json();
        setToast({ message: errData.error || "Failed to delete Quality record.", type: 'error' });
      }
    } catch (error) {
      console.error("Error deleting Quality record:", error);
      setToast({ message: "An error occurred while deleting the Quality record.", type: 'error' });
    }
  };
  ...
  const executeBulkDelete = async (ids: string[]) => {
    ...
    try {
      ...
      setToast({
        message: `Bulk delete completed: ${successCount} Quality records deleted, ${failCount} failed.`,
